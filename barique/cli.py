...
from barique import __version__  # noqa, ditto

CONTEXT_SETTINGS = dict(auto_envvar_prefix='BARIQUE', help_option_names=['-h', '--help'])
...
class Context(object):
    ...
    def exit(self, exit_code):
        self.vlog("Exiting barique with exit code [%d]" % exit_code)
        sys.exit(exit_code)
...
def name_to_command(parent, name):
    try:
        ...
        if parent:
            mod_name = 'barique.commands.%s.%s' % (parent, name)
        else:
            mod_name = 'barique.commands.cmd_' + name
...
class bariqueCLI(click.MultiCommand):

    def list_commands(self, ctx):
        # We pre-calculate this so it works more nicely within packaged
        # versions of barique. Please feel free to fix this?

        commands = ['init', 'file', 'task']
        return commands
...
@click.command(cls=bariqueCLI, context_settings=CONTEXT_SETTINGS)
...
@click.option(
    "-i",
    "--instance",
    help='Name of instance in %s. This parameter can also be set via the environment variable BARICADR_INSTANCE' % global_config_path(),
    default='__default',
    show_default=True,
    required=True
)
